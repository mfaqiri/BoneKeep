...
/obj/item/weapon/shield/wood/attack_hand_secondary(mob/user, params)
	...
	if(!overlays.len)
		...
		var/picked_name = input(user, "Choose a Heraldry", "OSSEUS", name) as null|anything in sortList(GLOB.IconStates_cache['icons/roguetown/weapons/wood_heraldry.dmi'])
		if(!picked_name)
			picked_name = "none"
...
/obj/item/weapon/shield/tower/metal/attack_hand_secondary(mob/user, params)
	...
	if(!overlays.len)
		...
		var/picked_name = input(user, "Choose a Heraldry", "OSSEUS", name) as null|anything in sortList(GLOB.IconStates_cache['icons/roguetown/weapons/shield_heraldry.dmi'])
		if(!picked_name)
			picked_name = "none"
