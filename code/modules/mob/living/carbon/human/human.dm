		...
		if(held_item.get_sharpness() && held_item.wlength == WLENGTH_SHORT)
			...
			if(has_stubble)
				...
				if(do_after(user, 5 SECONDS, src))
					...
					update_body()
					GLOB.osseus_round_stats[STATS_BEARDS_SHAVED]++
					if(dna?.species)
...
/mob/living/carbon/human/ZImpactDamage(turf/T, levels)
	...
	GLOB.osseus_round_stats[STATS_MOAT_FALLERS]-- // If you get your ankles broken you fall. This makes sure only those that DIDN'T get damage get counted.
	GLOB.osseus_round_stats[STATS_ANKLES_BROKEN]++
	var/chat_message
	switch(rand(1,4))
		if(1)
