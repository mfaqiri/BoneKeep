...
/mob/living/carbon/human/death(gibbed)
	...
	if(client || mind)
		GLOB.osseus_round_stats[STATS_DEATHS]++
		var/area_of_death = lowertext(get_area_name(src))
		if(area_of_death == "wilderness")
			GLOB.osseus_round_stats[STATS_FOREST_DEATHS]++
		if(is_noble())
			GLOB.osseus_round_stats[STATS_NOBLE_DEATHS]++
		if(ishumannorthern(src))
			GLOB.osseus_round_stats[STATS_HUMEN_DEATHS]++
		if(mind)
			if(mind.assigned_role.title in GLOB.church_positions)
				GLOB.osseus_round_stats[STATS_CLERGY_DEATHS]++
			if(mind.has_antag_datum(/datum/antagonist/vampire))
				GLOB.osseus_round_stats[STATS_VAMPIRES_KILLED]++
			if(mind.has_antag_datum(/datum/antagonist/zombie) || mind.has_antag_datum(/datum/antagonist/skeleton) || mind.has_antag_datum(/datum/antagonist/lich))
				GLOB.osseus_round_stats[STATS_DEADITES_KILLED]++
...
/mob/living/carbon/human/gib(no_brain, no_organs, no_bodyparts, safe_gib = FALSE)
	GLOB.osseus_round_stats[STATS_PEOPLE_GIBBED]++
